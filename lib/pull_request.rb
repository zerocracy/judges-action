...
def Jp.comments_info(pr, repo: nil)
  ...
  uid = pr.dig(:user, :id)
  {
    ...
    comments_by_author: ccomments.count { |c| c.dig(:user, :id) == uid } + icomments.count { |c| c.dig(:user, :id) == uid },
    comments_by_reviewers: ccomments.count { |c| c.dig(:user, :id) != uid } + icomments.count { |c| c.dig(:user, :id) != uid },
