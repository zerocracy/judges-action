...
class TestAddReviewComments < Jp::Test
  ...
  def test_rescues_forbidden_on_repo_lookup
    WebMock.disable_net_connect!
    rate_limit_up
    stub_github('https://api.github.com/repositories/42', status: 403, body: { message: 'Resource not accessible by integration' })
    fb = Factbase.new
    fact = fb.insert
    ...
    refute_nil(f)
    assert_nil(f['stale'], '403 is transient — seed fact must NOT be marked stale=repository; next cycle will retry the repo lookup')
  end
