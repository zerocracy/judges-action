...
class TestIsHumanOrRobot < Jp::Test
  ...
  def test_identify_user_as_bot_or_human
    ...
    fb = Factbase.new
    fb.with(where: 'github', who: 10, name: 'user0', stale: 'user_old')
      ...
      .with(where: 'github', who: 15, name: 'rultor')
      .with(where: 'github', who: 16, name: '0pdd')
      .with(where: 'github', who: 17, name: 'other_bot')
      .with(where: 'github', who: 18, name: 'user4')
    load_it('is-human-or-robot', fb)
    assert_equal(9, fb.all.size)
    assert_equal(2, fb.picks(is_human: 1).size)
    assert_equal(4, fb.picks(is_human: 0).size)
    assert(fb.one?(where: 'github', who: 10, name: 'user0', stale: 'user_old'))
