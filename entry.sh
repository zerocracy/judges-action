...
if [ "${SKIP_VERSION_CHECKING}" != 'true' ]; then
    resp=$(curl --connect-timeout 5 --max-time 10 --retry 5 --retry-delay 5 --retry-max-time 40 --silent "${auth_args[@]}" -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/zerocracy/judges-action/releases/latest || true)
